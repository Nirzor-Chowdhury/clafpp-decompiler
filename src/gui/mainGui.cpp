...
MainGui::MainGui() : wxFrame(nullptr, MainWindowID, "C Based D Compiler", wxPoint(50, 50), wxSize(800, 600))
{
	...
	llmMenu->Append(LLMGenerateFunctionCommentID, "Generate Function Comment");
	llmMenu->Bind(wxEVT_MENU,
		[&](wxCommandEvent&) -> void { LLMGenerateFunctionComment(); },
		LLMGenerateFunctionCommentID);

	llmMenu->Append(LLMGenerateLineCommentsID, "Generate Line Comments");
	llmMenu->Bind(wxEVT_MENU,
		[&](wxCommandEvent&) -> void { LLMGenerateLineComments(); },
		LLMGenerateLineCommentsID);

	llmMenu->Append(LLMSuggestVariableNamesID, "Suggest Variable Names");
	llmMenu->Bind(wxEVT_MENU,
		[&](wxCommandEvent&) -> void { LLMSuggestVariableNames(); },
		LLMSuggestVariableNamesID);

	llmMenu->Append(LLMAnalyzeComplexityID, "Analyze Complexity");
	llmMenu->Bind(wxEVT_MENU,
	...
	llmMenu->Append(LLMOpenSettingsID, "LLM Settings...");
	llmMenu->Bind(wxEVT_MENU,
...
void MainGui::OpenFile()
{
	...
	if (openFileDialog.ShowModal() != wxID_CANCEL)
	{
		...
		if (!filePath.empty())
		{
			...
			{
				this->SetTitle("C Based D Compiler");
			...
			this->SetTitle("C Based D Compiler - opened file " + fileName);
